...
part1() {
    ...
    find dist/ConeDevelopment.app/Contents/Resources/lib/ -name "*.py" -exec rm "{}" \;
    
    echo
    echo "= Done ="
    echo "Result: dist/ConeDevelopment.app"
}
...
part2() {
    ...
    revision=$(python -c "from conedevelopment_files import version; print version.getVersionFromIni('conedevelopment_files')")
    
    size=100000
    source=dist/ConeDevelopment.app
    title=ConeDevelopmentInstaller-${revision}
    applicationName=ConeDevelopment
    finalDMGName=ConeDevelopmentInstaller-${revision}
    pack=pack.temp.dmg
